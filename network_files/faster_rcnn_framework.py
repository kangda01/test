...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        features = self.backbone(images)  # 将图像输入backbone得到特征图
        ...
        images_shape = [images.shape[-2:] for i in range(images.shape[0])]
        detections, detector_losses = self.roi_heads(features, proposals, images_shape, targets)

        # 对网络的预测结果进行后处理（主要将bboxes还原到原图像尺度上）
        # detections = self.transform.postprocess(detections, images.image_sizes, original_image_sizes)
        detections = self.transform.postprocess(detections, images_shape, original_image_sizes)
